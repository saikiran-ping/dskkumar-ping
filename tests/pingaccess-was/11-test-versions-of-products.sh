...
testProductVersion() {
  ...
  # Positive check
  kubectl exec -n "$PING_CLOUD_NAMESPACE" "$POD_NAME" -c pingaccess -- sh -c \
  ...
  kubectl exec -n "$PING_CLOUD_NAMESPACE" "$POD_NAME" -c pingaccess -- sh -c \
    "unzip -p /opt/out/instance/lib/pingaccess-${PRODUCT_VERSION}.jar META-INF/maven/com.pingidentity.pingaccess/pingaccess-admin/pom.properties | grep -q 'version=0.0.0.0'" > /dev/null 2>&1
